...
class FluxCompositionPipeline(FluxInpaintPipeline):
    r"""
    ...
    def gen(
        ...
        orig_ref_height = ref_image.height
        orig_ref_width = ref_image.width
        ...
        ref_image = self.image_processor.preprocess(
            ref_image, height=orig_ref_height, width=orig_ref_width, crops_coords=crops_coords, resize_mode=resize_mode
        )
        ...
        # 5.1 Prepare main_latents in full size
        ...
        # print(f"orig_main_latents shape: {orig_main_latents.shape}")
        ...
        # 5.2 Prepare ref_latents in original size
        ref_latents, ref_noise, ref_image_latents, ref_latent_image_ids = self.prepare_latents(
            image=ref_image,
            timestep=latent_timestep,
            batch_size=batch_size * num_images_per_prompt,
            num_channels_latents=num_channels_latents,
            height=orig_ref_height,
            width=orig_ref_width,
            dtype=prompt_embeds.dtype,
            device=device,
            generator=generator,
            latents=ref_latents,
        ...
        # 5.3 Prepare segmented ref_latents in original size
        ...
        # prepare composite_image latents
        composite = init_image.clone()
        composite[:, :, y1:y2, x1:x2] = composite[:, :, y1:y2, x1:x2]*(resized_ref_mask_condition <= 0.5)\
                                        + resized_ref_masked_image*(resized_ref_mask_condition > 0.5)*(1-blend_ratio)\
                                        + composite[:, :, y1:y2, x1:x2]*(resized_ref_mask_condition > 0.5)*blend_ratio
        composite_latents=None
        composite_latents, composite_noise, composite_image_latents, composite_latent_image_ids = self.prepare_latents(
            image=composite,
            timestep=latent_timestep,
            batch_size=batch_size * num_images_per_prompt,
            num_channels_latents=num_channels_latents,
        ...
        if use_rf_inversion:
            ...
            inv_time = starter.elapsed_time(ender)
        else:
            composite_inv_latents = None
        ...
        # 5.6 Compose main_latents and ref_latents
        _trajectory_main = self._unpack_latents(main_latents.clone(), height, width, self.vae_scale_factor)
            
        bounding_box_mask=torch.zeros_like(_trajectory_main)
        bounding_box_mask[:,:,
                         2*y1//self.vae_scale_factor:2*y2//self.vae_scale_factor,
                         2*x1//self.vae_scale_factor:2*x2//self.vae_scale_factor] = 1
        bounding_box_mask=self._pack_latents(
            latents=bounding_box_mask,
            batch_size=batch_size * num_images_per_prompt,
            num_channels_latents=num_channels_latents,
            height = 2 * (int(height) // self.vae_scale_factor),
            width  = 2 * (int(width) // self.vae_scale_factor),
        ...
        if use_rf_inversion:
            ...
            shape = (batch_size, num_channels_latents, _height, _width)
            gen_latents = randn_tensor(shape, generator=generator, device=device, dtype=prompt_embeds.dtype)
            gen_latents = self._pack_latents(gen_latents, batch_size, num_channels_latents, _height, _width)
        # 6. Denoising loop warming up
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                gen_latents = (1 - bounding_box_mask) * init_latents_proper + bounding_box_mask * gen_latents
                ...
                if callback_on_step_end is not None:
                    ...
                    gen_latents = callback_outputs.pop("latents", latents)
