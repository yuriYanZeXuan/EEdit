...
def blur_source(latents, noise_scale, source,target,drag_class='copy'):
    assert drag_class in ['copy','cut'], "drag_class must be 'copy' or 'cut'"
    ...
    if drag_class == 'copy':
        ...
        if source.shape==target.shape:
            latents[0, :, source[:, 1], source[:, 0]] = latents[0, :, target[:, 1], target[:, 0]] * img_scale + \
            torch.randn_like(latents[0, :, target[:, 1], target[:, 0]]) * noise_scale
        else:
            # print('source and target shape not same')
            latents[0, :, source[:, 1], source[:, 0]] = torch.randn_like(latents[0, :, source[:, 1], source[:, 0]])
    return latents
